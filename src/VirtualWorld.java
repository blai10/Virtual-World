...
public final class VirtualWorld extends PApplet {
    ...
    */
        ...
        for (int i = row - 1; i <= row + 1; i++) {
            for (int j = col - 1; j <= col + 1; j++) {
                ...
                if (world.withinBounds(position)) {
                    Optional<NewEntity> treeOption = world.getOccupant(position);
                    if (treeOption.isPresent()){
                        NewEntity newTree = treeOption.get();
                        if (newTree instanceof Tree){
                            world.removeEntityAt(position);
                            Skeleton skeleton = new Skeleton("skeleton", position, imageStore.getImageList("skeleton"), 1, 1);
                        ...
                        NewEntity hole = new Hole("hole", position, imageStore.getImageList("hole"));
                        world.addEntity(hole);

                        Zombie zombie = new Zombie("zombie", position, imageStore.getImageList("zombie"), 1, 1);
    ...
    public static void scheduleActions(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        for (NewEntity entity : world.entities) {
           if (entity instanceof EntityAnimate){
               ((EntityAnimate) entity).scheduleActions(scheduler, world, imageStore);
           }
        }
    }
